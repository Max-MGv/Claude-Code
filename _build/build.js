...
const statblock = require('./lib/statblock');
...
function build(name) {
  ...
  // Step 4: Write creatures.js from all statblock linked notes
  console.log('\nGenerating creatures.js…');
  const creaturesData = [];
  for (const [, note] of linkedNotes) {
    if (render.detectNoteType(note.content) !== 'statblock') continue;
    const data = statblock.parseStatblock(note.content);
    const lore = statblock.extractLore(note.content);
    if (data) creaturesData.push({ ...data, _lore: lore || '' });
  }
  const creaturesFile = path.join(adventureDir, 'creatures.js');
  const creaturesJs =
    `// Auto-generated by _build/build.js — do not edit manually\n` +
    `window.AdventureCreatures = ${JSON.stringify(creaturesData, null, 2)};\n`;
  fs.writeFileSync(creaturesFile, creaturesJs, 'utf-8');
  console.log(`  ✓ creatures.js written (${creaturesData.length} creature(s))`);

  // Step 7: Report unresolved links
